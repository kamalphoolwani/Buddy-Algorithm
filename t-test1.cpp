...
{
	struct _pthread_v *tv = my_calloc(1, sizeof(struct _pthread_v));
...
#define my_memalign(A, S) my_malloc(S)
...
static void bin_alloc(struct bin *m, size_t size, unsigned r)
{
	...
	{
		/* memalign */
		if (m->size > 0) my_free(m->ptr);
		m->ptr = (unsigned char *)my_memalign(sizeof(int) << r, size);
	}
	else if (r < 20)
	{
		/* calloc */
		if (m->size > 0) my_free(m->ptr);
		m->ptr = (unsigned char *)my_calloc(size, 1);
#if TEST > 0
	...
	else if ((r < 100) && (m->size < REALLOC_MAX))
	{
		...
		m->ptr = (unsigned char *)my_realloc(m->ptr, size);
	}
	else
	{
		/* malloc */
		if (m->size > 0) my_free(m->ptr);
		m->ptr = (unsigned char *)my_malloc(size);
	}
	if (!m->ptr)
	{
...
#endif
{
	struct thread_st *st = (thread_st *)ptr;
	int i, pid = 1;
	...
	p.m = (struct bin *)my_malloc(st->bins * sizeof(*p.m));
	p.bins = st->bins;
	p.size = st->size;
	for (b = 0; b < p.bins; b++)
	{
...
#elif USE_THR
	if (!st->sp) st->sp = my_malloc(STACKSIZE);
	if (!st->sp) return -1;
...
int main(int argc, char *argv[])
{
	...
	st =(thread_st *) my_malloc(n_thr * sizeof(*st));
	if (!st) exit(-1);
